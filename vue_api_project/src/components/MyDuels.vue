<template>
    <div>
        <h1>Meus Duelos</h1>
        <router-link style="float: right;" :to="{name: 'adduel'}">Adicionar</router-link>
        <ul v-if="!hasDuels()">
            <li class="booking-card" v-for="duel in duels" v-bind:key="duel.id" :style='{ backgroundImage: `url(${duel.url})` }'>
                <div class="book-container">
                    <div class="content">
                        <router-link class="btn" :to="{name: 'items', params: {slug: duel.slug}}">Gerir</router-link>
                    </div>
                </div>
...
export default {
  ...
  getDuels() {
    const config = {
      method: 'get',
      url: '/api/duel?criador='+window.localStorage.getItem('id'),
      headers: {
